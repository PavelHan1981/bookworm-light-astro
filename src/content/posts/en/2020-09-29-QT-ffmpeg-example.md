---
title: "Simplest QT+ffmpeg Based Player Example"
slug: "2020-09-29-QT-ffmpeg-example"
description: "This article summarizes a simple example of developing a player using QT and ffmpeg."
...
- Based on [Setting up an FFmpeg Development Environment with QT Creator](https://www.pavelhan.tech/2020-09-24-qt-creator-ffmpeg), complete the QT Creator + FFmpeg development environment setup in a Windows environment, ensuring that interfaces from the FFmpeg library can be called correctly within QT Creator.

## UI Preparation:

- Create a new QT Widgets Application project in QT Creator and change its page class's base class to `QWidget`.
- Modify the UI file of the page in the project's Forms to place a `pushButton` and a `QLabel`. The decoded images from the player will be displayed in the `QLabel`'s `Pixmap`.
...
```c++
// Delay function, in milliseconds, does not block UI operations
void delay(int msec)
{
...
- [Simple Video Playback Implementation with Qt+FFmpeg](https://www.cnblogs.com/linuxAndMcu/p/12046600.html)